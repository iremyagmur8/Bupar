...
class HomepageController extends Controller
{
    ...
    public function index()
    {
        ...
        $cData->hakkimizda = category::where("id", 2)->first();
        $cData->category = category::where("id", 24)->first();
        $cData->testimonial = post::where("category_id", 24)->get();
        $cData->references = post::where('category_id', 5)->limit(6)->get();
    ...
    public function category(Request $request)
    {
        ...
        $cData->posts = post::where("category_id", $request->category_id)->where('publish_time', "<", date("Y-m-d H:i:s"))->orderByDesc('publish_time')->get();
    ...
    public function post(Request $request)
    {
        ...
        $cData->posts = post::where("category_id", $request->id)->orderBy('sort')->get();

        if (count($cData->posts) == 1) {
            $cData->posts = $cData->posts->first();
            return view('home.post', ['cData' => $cData]);
        } elseif (count($cData->posts) > 1) return view('home.post', ['cData' => $cData]);
    }

    public function contact(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->contact = category::where("id", 6)->first();
        return view('home.contact', ['cData' => $cData]);
    }

    public function referanslarimiz(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->categories = category::where('id', 5)->first();
        $cData->references = post::where('category_id', 5)->get();

        return view('home.referanslarimiz', ['cData' => $cData]);
    }
